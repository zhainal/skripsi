                            ...
                            <tbody>';
                                ...
                                $return .= '<tr>
                                    <td style="border-top:0px;">
                                        ...
                                        <b>
                                        '.$m['nama'].'
                                        '.(($v['aktif'] == 0) ? '<span class="text-error"><i class="icon-info-sign"></i> Mata Pelajaran Kelas Tidak Aktif' : '').'
                                        </b>

                                        <div id="modal-'.$v['id'].'" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                            <div class="modal-header">
                                                <h3 id="myModalLabel">Anda yakin ingin menghapus Mata Pelajaran Kelas ini?</h3>
                                            </div>
                                            <div class="modal-footer">
                                                <button class="btn btn-default" data-dismiss="modal" aria-hidden="true">Tidak</button>
                                                <a class="btn btn-danger" href="'.site_url('kelas/mapel_kelas/remove/'.$p['id'].'/'.$s['id'].'/'.$v['id'].'/'.enurl_redirect(current_url())).'">Ya</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>';
    ...
    function mapel_kelas($act = 'list', $segment_4 = '', $segment_5 = '', $segment_6 = '', $segment_7 = '')
    {
        switch ($act) {
            case 'aktifkan':
                ...
                $this->session->set_flashdata('edit-mapel-kelas-'.$kelas['id'], get_alert('success', 'Mata Pelajaran Kelas Berhasil Diaktifkan.'));
                redirect($uri_back);
            break;
            ...
            case 'remove':
                ...
                $this->session->set_flashdata('edit-mapel-kelas-'.$kelas['id'], get_alert('warning', 'Mata Pelajaran Kelas Berhasil Dihapus.'));
                redirect($uri_back);
            break;
            ...
            case 'add':
                ...
                if (!empty($_POST['add-mapel'])) {
                    if ($this->form_validation->run('mapel/add') == TRUE AND !is_demo_app()) {
                        ...
                        $this->session->set_flashdata('mapel', get_alert('success', 'Mata Pelajaran Baru Berhasil Ditambah.'));
                ...
                if ($this->form_validation->run('kelas/mapel_kelas/add') == TRUE AND !is_demo_app()) {
                    ...
                    $this->session->set_flashdata('mapel', get_alert('success', 'Mata Pelajaran Kelas Berhasil Disimpan.'));
                    redirect('kelas/mapel_kelas/add/'.$parent_id.'/'.$kelas_id.'/'.enurl_redirect($uri_back));
                }
            break;
